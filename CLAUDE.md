...
| `--fix-pos` | none | Hold positions at the target's residue, ColabDesign's syntax (`306-311,320`). **PDB residue numbers**, not sequence indices |
| `--fix-pos-inverse` | off | Design only the `--fix-pos` positions (ColabDesign's `inverse=True`) |
...
- **Position fixing**: `--fix-pos` follows `colabdesign.shared.prep.prep_pos` -- comma-separated segments, hyphen ranges, **PDB residue numbers**. The offset matters and differs per target (PHF 306-378, 6ELM 452-549), so `1-6` is refused with the valid range rather than fixing nothing; `parse_positions`/`designable_indices` in `utils.py` do the work and `mutate_sequence` takes a `designable` index list. Chain prefixes are refused because the chains share one designed sequence. A held position conflicting with `--initial-seq` is an error, not a silent overwrite. **A replacement search strategy must pass `designable` through**, or the option silently stops working
